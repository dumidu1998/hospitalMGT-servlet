...
<body class="colBg">
	...
	PatientHospitalizationDAO hospitalizationDAO = new PatientHospitalizationDAO(conn);
	PatientDAO patientDAO = new PatientDAO(conn);
	WardDAO wardDAO = new WardDAO(conn);
	DoctorDAO doctorDAO= new DoctorDAO(conn);
	ResultSet patients = patientDAO.getAllbyBranch((Integer) session.getAttribute("branch_id"));
	ResultSet wards=wardDAO.getAllbyBranch((Integer) session.getAttribute("branch_id"));
	ResultSet doctors = doctorDAO.getAllbyBranch((Integer) session.getAttribute("branch_id"));
	
	ResultSet hospitalizations =null;
	
	if ((Integer) session.getAttribute("role") == 1) {
		hospitalizations = hospitalizationDAO.getAll();
	} else {
		hospitalizations = hospitalizationDAO.getAllbyBranch((Integer) session.getAttribute("branch_id"));
	}
	...
	<div class="main-content">
		...
		<div class="wrapper mt-lg-3">
			...
			<form action="hospitalize" method="POST">
			<input type="hidden" name="branch" value="<%out.print((Integer) session.getAttribute("branch_id")); %>" />
						...
						<div class="row centerCont">
							...
							<div class="col col-lg-7">
								<select class="form-select" aria-label="Default select example"
										...
										while (patients.next()) {
										%>
										<option value=<%out.println(patients.getInt("patient_id"));%>>
											<%
											out.println(patients.getString("name"));
											%>
										</option>
										<%
										}
					...
					<div class="col col-lg-6 col-md-10 col-xs-11">
						<div class="row centerCont">
							...
							<div class="col col-lg-7">
								<select class="form-select" aria-label="Default select example"
										name="ward" required>
										<option selected hidden value="">Select the Ward</option>
										<%
										while (wards.next()) {
										%>
										<option value=<%out.println(wards.getInt("ward_id"));%>>
											<%
											out.println(wards.getString("name"));
											%>
										</option>
										<%
										}
				...
				<div class="row centerCont">
					<div class="col col-lg-6 col-md-10 col-xs-11">
						<div class="row centerCont">
							...
							<div class="col col-lg-7">
								<textarea rows="4" cols="12" class="form-control" name="remarks"></textarea>
							</div>
						</div>
					</div>
					<div class="col col-lg-6 col-md-10 col-xs-11">
				...
				<div class="row centerCont">
					<div class="d-grid gap-2 col-2 mx-auto pt-3">
						<button  class="btn btnSubmit btn-sm ">Confirm Hospitalization</button>
					</div>
				</div>
				</form>
			</div>
				...
				<div class="row centerCont ">
					<div class="col col-lg-11 col-md-10 col-xs-11">
						...
						<table class="table table-primary table-hover table-responsive">
							...
							<tbody>
							<%
								while (hospitalizations.next()) {
								%>
								<tr class="table-light">
									<th scope="row"><%out.println(hospitalizations.getString("p_name"));%></th>
									<td><%out.println(hospitalizations.getString("ward_name"));%></td>
									<td><%out.println(hospitalizations.getString("branch_name"));%></td>
									<td><%out.println(hospitalizations.getDate("date_admitted"));%></td>
									<td>
										<ul class="flexList">
											<li>
											<a href="hospitalize?id=<%out.println(hospitalizations.getInt("hospitalization_id"));%>" >
											<button>
